<template>
  <div class="role-mgmt-container">
    <n-card title="角色权限管理" :bordered="false" class="ai-glass-card">
      <template #header-extra>
        <n-button type="primary" secondary @click="handleAddRole">
      ...
      <n-table :single-line="false">
        ...
        <tbody>
          <tr v-for="role in roles" :key="role.id">
            ...
            <td>
              <n-space>
                <n-button size="small" ghost type="primary" @click="handleEditRole(role)">
                  编辑
                </n-button>
                <n-button size="small" ghost type="info" @click="handleEditPerms(role)">
                  分配权限
                </n-button>
                <n-button size="small" ghost type="error" :disabled="role.code === 'ADMIN'" @click="handleDeleteRole(role)">
                  删除
                </n-button>
              </n-space>
            </td>
    ...
    >
      <n-form ref="formRef" :model="formValue" :rules="rules" style="margin-top: 20px">
        ...
        <n-form-item label="角色代码" path="code" v-if="!isEdit">
...
const resetForm = () => {
  formValue.value = { name: '', code: '', description: '' }
}
...
<style scoped>
.role-mgmt-container { padding: 10px; }
.ai-glass-card {
